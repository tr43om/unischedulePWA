...
const ScheduleGrid = ({ scheduleData }: ScheduleGridProps) => {
  ...
  return (
    <table className=" relative -ml-2 table w-[calc(100%_+_0.5rem)] table-fixed  border-separate border-spacing-2 rounded-md">
      {/* head */}
      <thead>
        <tr className="relative">
          <th className=" sticky -top-2 left-0 z-10 w-[75px] min-w-[200px] justify-items-center bg-neutral">
            <ClockIcon width={20} height={20} className="mx-auto text-white" />
          </th>
          {weeklySchedule.map((day) => (
            <th
              key={day.timestamp + "th"}
              className="sticky -top-2 left-0 z-10 bg-neutral"
            >
      ...
      <tbody>
        {weeklyTimetable.map((time, i) => (
          <tr key={`${time}`} className=" border-neutral">
            <td className="   rounded-md bg-neutral/[0.15]  font-bold  dark:bg-neutral/30  dark:text-gray-500">
              {time}
            </td>
            {weeklySchedule.map(({ lessons, timestamp }) => {
              ...
              return (
                <td
                  key={`${timestamp}-day`}
                  className={`m-6 whitespace-normal rounded-md bg-neutral/[0.15]  align-top dark:border-gray-600 dark:bg-neutral/30`}
                >
                  <>
